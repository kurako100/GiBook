...
- ユーザーの招待（招待キーにて入室）
- 書籍の検索（書籍検索APIを使用）
- 書籍情報の投稿（書籍情報：書籍URL、あらすじ、投稿者のコメント）
  - 書籍情報を共有本棚に追加（※書籍情報を投稿したら自動で本棚に追加される仕様）
- 共有本棚に追加された書籍情報の閲覧・ブックマーク・リアクション（スタンプ、コメント）
...
3. 書籍情報にブックマークとステータスを追加：ステータス「貸し借り可能か」「現在誰が持っているか」「予約（次に読みたい人）」
...
- [ ] ログアウト機能
- [ ] 図書室作成機能（共有本棚の作成）
- [ ] 招待機能（招待キーの生成および入力による入室）
- [ ] 書籍検索機能（書籍検索APIを使用）
- [ ] 書籍情報投稿機能（書籍URL、あらすじ、投稿者コメントの入力）
- [ ] 共有本棚への書籍情報追加機能（投稿と連動）
- [ ] 書籍情報閲覧機能（共有本棚内の閲覧）
- [ ] 書籍ブックマーク機能
- [ ] 書籍リアクション機能（スタンプ、コメント）

### 未ログインでも閲覧または利用できるページ
- [ ] 書籍情報の閲覧機能（未ログインでも閲覧可能）

### メールアドレス・パスワード変更確認項目
- [ ] メールアドレス（専用のページで変更できる仕組みか？）
- [ ] パスワード（専用のページで変更できる仕組みか？）

### 各画面の作り込み
- [ ] 必要なボタンやフォームが確認できるくらい作り込めているか？
