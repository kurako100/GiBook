...
- メールアドレス、LINE認証（※）の２種類の登録方法（※LINE通知を使用するため）
- ログイン、ログアウト
...
- 書籍情報の投稿（書籍情報：書籍URL、あらすじ、投稿者のコメント）
  - 書籍情報を共有本棚に追加（※投稿したら自動で本棚に追加される仕様）
- 共有本棚に追加された書籍情報の閲覧・ブックマーク・リアクション（スタンプ、コメント）

  
### 本リリース
1. つぶやき機能（図書室内でのチャット機能）：クローズドなXのようなイメージ。本の感想などを気軽につぶやくことができる
2. 書籍情報にステータス追加：「貸し借り可能か」「現在誰が持っているか」「予約（次に読みたい人）」
3. LINE通知機能：図書室内でのやりとりを通知（書籍情報の投稿、リアクション、つぶやき、ステータスの更新、など）
- その他
  - メアド更新機能
  - パスワード再設定機能
  - 利用規約ページ作成
